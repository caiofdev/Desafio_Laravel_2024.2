    ...
    <x-app-layout>
        ...
        <x-slot name="header">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight text-white flex justify-center">
                {{ __('Create a new manager:') }}
            </h2>
        </x-slot>

        <div class="py-10">

            <form action="{{ route('manager.store') }}" method="POST" enctype="multipart/form-data" class="max-w-3xl mx-auto bg-slate-900 p-8 rounded-lg shadow-md">
                @csrf
